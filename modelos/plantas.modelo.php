...
class ModeloPlantas{
	...
	static public function mdlMostrarPlantasPre($tabla, $item, $valor){

		if($item != null){

			$stmt = Conexion::conectar()->prepare("SELECT * FROM planta WHERE id='$valor'");
			$stmt -> execute();
			return $stmt -> fetch();

		}else{

			$stmt = Conexion::conectar()->prepare("SELECT * FROM planta");
			$stmt -> execute();
			return $stmt -> fetchAll();
