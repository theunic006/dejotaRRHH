	...
	public function mostrarTablaPersonal(){
    	...
    	$orden = "id";
		  ...
		  for($i = 0; $i < count($personal); $i++){
			...
			$examenes2 = "<div class='btn-group'><button class='btn btn-primary btnEditarPersonal' data-toggle='modal' data-target='#modalEditarPersonal' idPersonal='".$personal[$i]["id"]."'><i class='fa fa-edit'></i></button><button class='".$examen." btnEditarExamen' data-toggle='modal' data-target='#modalEditarExamen' idExamen='".$personal[$i]["id"]."'><i class='fa fa-user-md'></i> EM</button><button class='".$induccion." btnEditarInduccion' data-toggle='modal' data-target='#modalEditarInduccion' idInduccion='".$personal[$i]["id"]."'><i class='fa fa-odnoklassniki-square'></i> ID</button></div>";

		  	$botones = "<div class='btn-group'><button class='btn ".$observacion." agregarPersonal recuperarBoton' idPersonal='".$personal[$i]["id"]."'><i class='fa ".$seleccionar."'></i> ".$ok."</button></div>"; 
